  ...
  return (
    <CustomParallaxScrollView
      ...
      headerYoutubeEmbedUrl={
        anime?.data.trailer.embed_url ?? 'https://www.youtube.com/embed/Gl4bf3Gppyo?si=YCeKHdGgHs4nwaZz'
      }
    >
      ...
      <View style={styles.card}>
        <View>
          <Text>
            <Text style={[styles.fontText]}>Mùa:</Text>{' '}
            <Text style={[styles.text]}>
              {getSeasonInVietnamese(anime?.data.season ?? '')} {anime?.data.year ?? 'Không rõ'}
            </Text>
          </Text>
          <Text>
            <Text style={[styles.fontText]}>Điểm:</Text>{' '}
            <Text style={[styles.text]}>{anime?.data.score ?? 'Không rõ'}/10</Text>
          </Text>
          <Text>
            <Text style={[styles.fontText]}>Số tập:</Text>{' '}
            <Text style={[styles.text]}>{anime?.data.episodes ?? 'Chưa rõ'}</Text>
          </Text>
          <Text>
          ...
          <Text>
            <Text style={[styles.fontText]}>Tình trạng: </Text>{' '}
            <Text style={[styles.text]}>{anime?.data.status ?? 'Không rõ'}</Text>
          </Text>
        </View>
      </View>
      <View style={styles.card}>
        <Collapsible title='Nội dung'>
          <ThemedText>{anime?.data.synopsis ?? 'Quá lỏ nên tạm chưa có nội dung'}</ThemedText>
